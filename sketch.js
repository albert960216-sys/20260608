...
function setup() {
  ...
  // 啟動持續偵測偵測模式
  if (ml5) {
    // 初始化模型，暫時不傳入 video，等 modelLoaded 再開始偵測
...
function draw() {
  ...
  textSize(20);
  if (!modelReady) {
    fill(255, 255, 0); // 載入中顯示黃色
    text("模型狀態：載入中...", 20, 20);
  } else {
    if (predictions.length > 0) {
      fill(0, 255, 0); // 偵測到手顯示綠色
      text("模型狀態：已就緒 | 偵測：已找到手 ✅", 20, 20);
    } else {
      fill(255, 0, 0); // 沒偵測到手顯示紅色警告
      text("模型狀態：已就緒 | 偵測：未發現手 ❌ (請揮手試試)", 20, 20);
    }
  }
  pop();
}
